  ...
  return (
    <div className="h-[100vh] flex flex-col">
            ...
            {Array.from({ length: 8 }).map((_, index) => (
              <Card className="w-full h-16">
                <CardContent className="flex items-center justify-center">
                  <span className="text-4xl font-semibold dark:text-white">{index + 1}</span>
                </CardContent>
              </Card>
            ))}
          </section>
      ...
      <footer className="sticky bottom-0 w-full bg-white text-black dark:bg-black dark:text-white flex justify-around align-center">
        <Button className="h-16" variant="ghost">Home</Button>
        <Button  className="h-16"  variant="ghost">Search</Button>
        <Button  className="h-16"  variant="ghost">Library</Button>
        <Button  className="h-16"  variant="ghost">Create</Button>
      </footer>
    </div>
  );
}
